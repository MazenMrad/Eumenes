...
KEEPALIVE_STATUS_FILE = Path("/tmp/eumenes-keepalive-status.json")
...
def cf_request(method, path, token, body=None, content_type="application/json"):
    ...
    try:
        ...
        try:
            ...
            errors = err.get("errors") or [{"message": "Unknown error"}]
...
def slugify(value):
    ...
    return (cleaned or "eumenes-keepalive")[:63].rstrip("-")
...
def derive_worker_name():
    ...
    return "eumenes-keepalive"


def render_worker(target_url):
...
async function ping(source) {{
  ...
  try {{
    const response = await fetch(TARGET_URL, {{
      method: "GET",
      headers: {{
        "user-agent": "EumenesKeepAlive/1.0",
...
def write_keepalive_status(payload):
    payload = {
        **payload,
...
def resolve_account_and_subdomain(api_token):
    ...
    if not account_id:
        ...
        if not accounts:
            raise RuntimeError("No Cloudflare account found for this token")
...
def setup(api_token, account_id, subdomain):
    enabled = os.environ.get("CLOUDFLARE_KEEPALIVE_ENABLED", "true").strip().lower()
    if enabled in ("0", "false", "no", "off"):
        write_keepalive_status({"configured": False, "status": "disabled", "message": "Keep-awake is disabled via CLOUDFLARE_KEEPALIVE_ENABLED"})
        return ""
    ...
    cron = os.environ.get("CLOUDFLARE_KEEPALIVE_CRON", "*/5 * * * *").strip()
    ...
    source = render_worker(target_url)
    ...
    write_keepalive_status({
        ...
        "cron": cron,
        "message": f"CF Worker cron pings {target_url} every {cron}",
    })
    return worker_url
...
def read_status():
    try:
        if KEEPALIVE_STATUS_FILE.exists():
            return json.loads(KEEPALIVE_STATUS_FILE.read_text(encoding="utf-8"))
    except Exception:
        pass
...
def main():
    ...
    if not api_token:
        write_keepalive_status({"configured": False, "status": "skipped", "message": "CLOUDFLARE_WORKERS_TOKEN not set"})
        return 0
    try:
        account_id, subdomain = resolve_account_and_subdomain(api_token)
        setup(api_token, account_id, subdomain)
        return 0
    except Exception as exc:
        print(f"Keepalive setup failed: {exc}", file=sys.stderr)
